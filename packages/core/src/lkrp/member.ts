...
export function decodeMember(input: string): KlaxonMember {
  ...
  try {
    ...
    throw new KlaxonError("MEMBER_MALFORMED", "KLAXON_MEMBER is not valid JSON or base64 JSON", {
      cause,
    });
  }
  const parsed = KlaxonMemberSchema.safeParse(json);
  if (!parsed.success)
    throw new KlaxonError("MEMBER_MALFORMED", "KLAXON_MEMBER failed schema validation");
